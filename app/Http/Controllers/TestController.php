...
class TestController extends Controller
{
    ...
    {
        return view('test.index',['context' => $this]);
    
    }

    public function test_filter(Request $request)
    {
                ...
                if($searchValue != ''){
                  $query_set_value .= " AND (connected like '%".$searchValue."%' )";
                }
                if ($order_datefrom != '') {
                ...
                $orderBy .= " LIMIT ".$_POST['start']." ,".$_POST['length'];
                 $array = \DB::select("SELECT *,id AS chk FROM test $query_set_value $orderBy");
                $totalarray = \DB::select("SELECT *,id AS chk FROM test $query_set_value");
                ...
                if($array != NULL){
                    foreach($array AS $key => $row){
                        ...
                        $rowData->id = $i;						
						$rowData->connected = $row->connected;						                   
                        $rowData->disconnected = $row->disconnected;
						$rowData->firmware = $row->firmware;
                         $rowData->batteryLevel = $row->batteryLevel;
                         $rowData->temperatureValue = $row->temperatureValue;
                         $rowData->dateAndTimeValue = $row->dateAndTimeValue;
                         $rowData->alertLevel = $row->alertLevel;
                         $rowData->alertStatus = $row->alertStatus;
                         $rowData->manufacturerName = $row->manufacturerName;
                         $rowData->modelNumber = $row->modelNumber;
