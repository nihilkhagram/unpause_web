...
class ReportController extends Controller
{
    ...
    {
        return view('report.index',['context' => $this]);
    ...
    public function report_filter(Request $request)
    {
				...
				if(isset($_REQUEST['search_input'])){
                 ...
                 $order_datefrom = isset($rowData['order_datefrom']) ? $rowData['order_datefrom'] : '';
		        $order_dateto = isset($rowData['order_dateto']) ? $rowData['order_dateto'] : '';
		
            
                $page = isset($rowData['page']) ? $rowData['page'] : '';
            
                
                $Created_by = isset($rowData['Created_by']) ? $rowData['Created_by'] : '';
                ...
                $orderBy .= " LIMIT ".$_POST['start']." ,".$_POST['length'];
                 $array = \DB::select("SELECT *,id AS chk FROM report $query_set_value $orderBy");
                // print_r($array);
                // exit;
                 $totalarray = \DB::select("SELECT *,id AS chk FROM report $query_set_value");
                ...
                if($array != NULL){
                    foreach($array AS $key => $row){
                        ...
                        $rowData->birth_year = $row->birth_year;
                        $rowData->ethnic_origin = $row->ethnic_origin;
                        $rowData->country_of_residence = $row->country_of_residence;
                        $rowData->height = $row->height;
                        $rowData->weight = $row->weight;
                        $rowData->changes_weight = $row->changes_weight;
                        $rowData->often_excercise = $row->often_excercise;
                        $rowData->excercise_patterns_changes = $row->excercise_patterns_changes;
                        $rowData->often_consume_alcohol = $row->often_consume_alcohol;
                        $rowData->smoke = $row->smoke;
                        $rowData->period = $row->period;
                        $rowData->last_period_date = $row->last_period_date;
                        $rowData->hot_flashes = $row->hot_flashes;
                        $rowData->occur_hf = $row->occur_hf;
                        $rowData->night_sweats = $row->night_sweats;
                        $rowData->cold_flashes = $row->cold_flashes;
                        $rowData->occur_cf = $row->occur_cf;
                        $rowData->affect_sleep = $row->affect_sleep;
                        $rowData->is_symptoms1 = $row->is_symptoms1;
                        $rowData->is_symptoms2 = $row->is_symptoms2;
                        $rowData->experience_symptoms = $row->experience_symptoms;
                        $rowData->spoken_abt_menopausal = $row->spoken_abt_menopausal;
                        $rowData->cycle_lengths_changed_recently = $row->cycle_lengths_changed_recently;
                        $rowData->duration_period_changed = $row->duration_period_changed;
                        $rowData->bleeding_been_heavier_period = $row->bleeding_been_heavier_period;
                        $rowData->occur_ns = $row->occur_ns;
                        $rowData->nhs_or_private_healthcare = $row->nhs_or_private_healthcare;
                        $rowData->whom_did_you_consult = $row->whom_did_you_consult;
                        $rowData->received_diagnosis_healthcare_professional = $row->received_diagnosis_healthcare_professional;
                        $rowData->on_hormone_replacement_therapy = $row->on_hormone_replacement_therapy;
                        $rowData->which_form_of_HRT_are_you_on = $row->which_form_of_HRT_are_you_on;
                        $rowData->type_of_HRT_routine_are_you_on = $row->type_of_HRT_routine_are_you_on;
                        $rowData->type_of_cyclic_HRT_routine_are_you_on = $row->type_of_cyclic_HRT_routine_are_you_on;

                        
                        

                        // $Created_by = User::where('id',$row->Created_by)->first();
    					// $rowData->Created_by = $Created_by['email'];

                        // $Created_by = User::where('id',$row->login_id)->first();
						// $rowData->login_id = $Created_by['email'];
