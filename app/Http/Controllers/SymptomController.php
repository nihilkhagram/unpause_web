...
class SymptomController extends Controller
{
    ...
    {
        return view('symptom.index',['context' => $this]);
    
    }

    public function symptom_filter(Request $request)
    {
                ...
                if($searchValue != ''){
                  $query_set_value .= " AND (symptom_id like '%".$searchValue."%' )";
                }
                if ($order_datefrom != '') {
                ...
                $orderBy .= " LIMIT ".$_POST['start']." ,".$_POST['length'];
                 $array = \DB::select("SELECT *,id AS chk FROM symptom $query_set_value $orderBy");
                // print_r($array);
                // exit;
                 $totalarray = \DB::select("SELECT *,id AS chk FROM symptom $query_set_value ");
                ...
                if($array != NULL){
                    foreach($array AS $key => $row){
						...
						$rowData->key_name = $row->key_name;
                         $rowData->is_selected = $row->is_selected;
                         $rowData->source = $row->source;
                         $rowData->source_color = $row->source_color;
